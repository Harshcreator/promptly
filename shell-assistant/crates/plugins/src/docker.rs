...
impl Plugin for DockerPlugin {
    ...
    fn handle(&self, input: &str) -> Option<CommandResult> {
        ...
        if input_lower.contains("list") && (input_lower.contains("container") || input_lower.contains("all container")) {
        ...
        if input_lower.contains("pull") && input_lower.contains("image") {
            ...
            return Some(CommandResult {
                ...
                explanation: "Pulls a Docker image. You'll need to specify the image name.".to_string(),
                executed: false,
                output: None,
            });
        }
        ...
        if (input_lower.contains("run") || input_lower.contains("start")) && 
           (input_lower.contains("container") || input_lower.contains("image")) {
            ...
            return Some(CommandResult {
                ...
                explanation: "Runs a Docker container. You'll need to specify the image name.".to_string(),
                executed: false,
                output: None,
            });
        }
        ...
        if input_lower.contains("stop") && input_lower.contains("container") {
            ...
            return Some(CommandResult {
                ...
                explanation: "Stops a running container. You'll need to specify the container ID or name.".to_string(),
                executed: false,
                output: None,
            });
        }
        ...
        if (input_lower.contains("remove") || input_lower.contains("delete")) && input_lower.contains("container") {
            ...
            return Some(CommandResult {
                ...
                explanation: "Removes a container. You'll need to specify the container ID or name.".to_string(),
                executed: false,
                output: None,
            });
        }
        ...
        // Removing images
        if (input_lower.contains("remove") || input_lower.contains("delete")) && input_lower.contains("image") {
            if let Some(image) = extract_image_name(input) {
                return Some(CommandResult {
            ...
            return Some(CommandResult {
                ...
                explanation: "Removes a Docker image. You'll need to specify the image ID or name.".to_string(),
                executed: false,
                output: None,
            });
        }
        ...
        if input_lower.contains("compose") && input_lower.contains("down") {
            return Some(CommandResult {
                ...
                explanation: "Stops and removes all services defined in docker-compose.yml.".to_string(),
                executed: false,
                output: None,
            });
        }
...
fn extract_image_name(input: &str) -> Option<String> {
    ...
    if idx + 1 < words.len() {
        Some(words[idx + 1].trim_matches(|c: char| !c.is_alphanumeric() && c != '.' && c != '_' && c != ':' && c != '/').to_string())
    } else {
        None
    }
}
...
fn extract_container_name(input: &str) -> Option<String> {
    ...
    if idx + 1 < words.len() {
        Some(words[idx + 1].trim_matches(|c: char| !c.is_alphanumeric() && c != '.' && c != '_' && c != '-').to_string())
    } else {
        None
    }
}
